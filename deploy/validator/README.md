...
Everything official is already in `.env.example` and is kept current by CI
(`scripts/update-release-values.sh` on each release): the pinned node image +
digest, the genesis sha256, the `sovrd` binary sha256, the seeds, and the signed
snapshot bucket + cosign key. You do **not** paste any of these by hand. The
